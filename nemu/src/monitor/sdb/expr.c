...
#define NR_REGEX ARRLEN(rules) //宏NR_REGEX，表示规则数组rules的长度
...
//定义了一个记号数组tokens和一个整数nr_token，用于存储词法分析得到的记号序列和记号的数量
...
static bool make_token(char *e) {//函数make_token(char *e)，用于对给定输入字符串进行分词
  ...
  while (e[position] != '\0') {//函数进入一个while循环，循环条件是尚未到达输入字符串的末尾（e[position] != '\0'）
    ...
    //在循环内部，函数遍历一个包含正则表达式（re）和相应规则的数组。目标是找到当前位置在输入字符串中的匹配项
      ...
      //&pmatch是一个存储匹配结果的结构体
      //0表示不使用任何标志位
      //pmatch.rm_so表示匹配结果的起始位置
      //如果起始位置为0，则表示整个子字符串与正则表达式完全匹配。通过这个条件，代码可以过滤掉部分匹配或包含匹配的情况
